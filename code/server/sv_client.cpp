...
void SV_DropClient( client_t *drop, const char *reason ) {
	int		i;
	challenge_t	*challenge;
	const qboolean isBot = (drop->netchan.remoteAddress.type == NA_BOT) ? qtrue : qfalse;
			...
			if(!Q_stricmp(pakptr + 1, "pk3"))
			{
				...
				for(curindex = 0; curindex < numRefPaks; curindex++)
				{
					if(!FS_FilenameCompare(Cmd_Argv(curindex), pakbuf))
					{
						...
						idPack = missionPack;
#endif
						idPack = (idPack || FS_idPak(pakbuf, BASEGAME, NUM_ID_PAKS)) ? qtrue : qfalse;
...
static void SV_VerifyPaks_f( client_t *cl ) {
	...
	if ( sv_pure->integer != 0 ) {
		...
		bGood = (FS_FileIsInPAK("vm/cgame.qvm", &nChkSum1) == 1) ? qtrue : qfalse;
		if (bGood)
			bGood = (FS_FileIsInPAK("vm/ui.qvm", &nChkSum2) == 1) ? qtrue : qfalse;
...
void SV_UserinfoChanged( client_t *cl ) {
...
#endif
	{
		...
		cl->hasVoip = (!Q_stricmp( val, "opus" )) ? qtrue : qfalse;
	}
#endif
