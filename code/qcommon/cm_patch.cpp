	...
	for ( i = 0 ; i < grid->width - 1 ; i++ ) {
		for ( j = 0 ; j < grid->height - 1 ; j++ ) {
			...
			if ( gridPlanes[i][j][0] == gridPlanes[i][j][1] ) {
				...
				facet->borderNoAdjust[0] = noAdjust[EN_TOP] ? qtrue : qfalse;
				facet->borderPlanes[1] = borders[EN_RIGHT];
				facet->borderNoAdjust[1] = noAdjust[EN_RIGHT] ? qtrue : qfalse;
				facet->borderPlanes[2] = borders[EN_BOTTOM];
				facet->borderNoAdjust[2] = noAdjust[EN_BOTTOM] ? qtrue : qfalse;
				facet->borderPlanes[3] = borders[EN_LEFT];
				facet->borderNoAdjust[3] = noAdjust[EN_LEFT] ? qtrue : qfalse;
				...
				facet->borderNoAdjust[0] = noAdjust[EN_BOTTOM] ? qtrue : qfalse;
				facet->borderPlanes[1] = borders[EN_LEFT];
				facet->borderNoAdjust[1] = noAdjust[EN_LEFT] ? qtrue : qfalse;
