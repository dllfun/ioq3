...
void QDECL Com_Error( int code, const char *fmt, ... ) {
	...
	restartClient = (com_gameClientRestarting && !( com_cl_running && com_cl_running->integer )) ? qtrue : qfalse;
...
int Com_EventLoop( void ) {
	...
	while ( 1 ) {
		...
		{
			case SE_KEY:
				CL_KeyEvent( ev.evValue, ev.evValue2 ? qtrue : qfalse, ev.evTime );
			break;
			case SE_CHAR:
...
void Com_GameRestart(int checksumFeed, qboolean disconnect)
{
	...
	if(!com_gameRestarting && com_fullyInitialized)
	{
		...
		com_gameClientRestarting = com_cl_running->integer ? qtrue : qfalse;
...
static void Com_DetectAltivec(void)
{
	...
	if (com_altivec->integer) {
		...
		if (!detected) {
			altivec = ( Sys_GetProcessorFeatures( ) & CF_ALTIVEC ) ? qtrue : qfalse;
			detected = qtrue;
		}
...
qboolean Com_IsVoipTarget(uint8_t *voipTargets, int voipTargetsSize, int clientNum)
{
	...
	if(index < voipTargetsSize)
		return (voipTargets[index] & (1 << (clientNum & 0x07))) ? qtrue : qfalse;
