...
static int CG_TeamScoreboard( int y, team_t team, float fade, int maxClients, int lineHeight ) {
	...
	for ( i = 0 ; i < cg.numScores && count < maxClients ; i++ ) {
		...
		CG_DrawClientScore( y + lineHeight * count, score, color, fade, (lineHeight == SB_NORMAL_HEIGHT) ? qtrue : qfalse);
	...
	if (!localClient) {
		...
		for ( i = 0 ; i < cg.numScores ; i++ ) {
			if ( cg.scores[i].client == cg.snap->ps.clientNum ) {
				CG_DrawClientScore( y, &cg.scores[i], fadeColor, fade, (lineHeight == SB_NORMAL_HEIGHT) ? qtrue : qfalse);
				break;
			}
		}
	}
